...
| [`src/lastfm_etl/transform/chart.py`](src/lastfm_etl/transform/chart.py) | ~670 | **Bölüm 0:** `.get()` zinciri vs `[]` (hangisi ne zaman), `isinstance` ve tip kapısı, `enumerate` ve sayfa içi index, `set` + tuple anahtar ve hashable, `dict.setdefault` ve argümanın her çağrıda hesaplanması, `a or b` varsayılanı ve falsy tuzağı, aware vs naive `datetime` ve `Z` eki. **Bölüm 1:** sabitin isimle taşıdığı karar, exception hiyerarşisinin derinliği ne kadar olmalı, `frozen` dataclass'ın sığ olması, dar `try` kuralı ve yakalanmayan hata tipinin bir iddia oluşu, `rank` formülü ve `perPage`'in neden sunucudan okunduğu, tek geçiş = yapısal FK bütünlüğü, dedup sırası ve orphan dimension, mutable default argument tuzağı, sessiz elemenin maliyeti |
